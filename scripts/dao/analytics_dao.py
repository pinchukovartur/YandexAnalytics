...
class AnalyticsDB:
    ...
    def insert_error(self, error):
        ...
        try:
            self.connection.cursor().execute(query, (
                ...
                error["device_manufacturer"], error["device_model"], error["device_type"], error["error_datetime"],
                error["error"], error["error_id"], error["error_receive_datetime"],
    ...
    def insert_installation(self, install):
        ...
        try:
            self.connection.cursor().execute(query, (
                ...
                install["install_ipv6"], install["install_datetime"], install["install_receive_datetime"],
                install["click_datetime"], install["click_ipv6"], install["click_id"],
    ...
    def insert_crashes(self, crash):
        query = "INSERT INTO analytics.crashes (" \
                "android_id,app_package_name,app_version_name,appmetrica_device_id, city,connection_type," \
                "country_iso_code,crash,crash_datetime,crash_group_id,crash_id,crash_receive_datetime," \
                "crash_receive_timestamp,crash_timestamp,device_locale,device_manufacturer,device_model,device_type,"\
                "google_aid,ios_ifa,ios_ifv,mcc,mnc,operator_name,os_name,os_version,windows_aid) VALUES" \
        ...
        try:
            self.connection.cursor().execute(query, (
                ...
                crash["country_iso_code"], crash["crash"], crash["crash_datetime"],
                crash["crash_group_id"], crash["crash_id"], crash["crash_receive_datetime"],
                ...
                crash["google_aid"], crash["click_ipv6"], crash["click_id"],
                crash["click_user_agent"], crash["click_url_parameters"], crash["click_timestamp"],
                crash["google_aid"], crash["ios_ifa"], crash["ios_ifv"],
                crash["mcc"], crash["mnc"], crash["operator_name"],
