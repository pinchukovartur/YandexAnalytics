...
class AnalyticsDB:
    ...
    def insert_error(self, error):
        ...
        try:
            ...
            print(error_str)
            print(error["android_id"], error["app_package_name"], error["app_version_name"])
            self.connection.rollback()
    ...
    def insert_installation(self, install):
        query = "INSERT INTO analytics.installations (" \
                ...
                "country_iso_code,device_locale,device_manufacturer," \
                "device_model,device_type, match_type," \
                "is_reinstallation,install_receive_timestamp,install_timestamp," \
                "install_ipv6, install_datetime,install_receive_datetime," \
                "click_datetime,click_ipv6,click_id," \
                "click_user_agent, click_url_parameters, click_timestamp," \
                "google_aid,ios_ifa,ios_ifv,mcc," \
                "mnc,operator_name,os_name," \
                "os_version,tracking_id," \
                "tracker_name,publisher_name,publisher_id" \
